...
export default function Home() {
  ...
  return (
    <div className="min-h-screen pb-20">
      {/* NAVBAR CORREGIDA: Buscador a la derecha en PC */}
      <nav className="bg-white/70 backdrop-blur-xl sticky top-0 z-40 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6 py-4 md:py-0 md:h-20 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          {/* 1. LOGO: Izquierda en PC, Arriba en Móvil */}
          ...
          {/* CONTENEDOR DERECHO: Buscador + Botón (Se apilan en móvil, se alinean en PC) */}
          <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            
            {/* 2. BUSCADOR: A la par del botón en PC, Ancho total en móvil */}
            <div className="relative w-full md:w-80">
              <input 
                type="text" 
                placeholder="Buscar colección..." 
                className="bg-slate-100 border-none rounded-2xl py-3 md:py-2.5 pl-11 pr-5 w-full text-sm focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all"
                value={search}
            ...
            {/* 3. BOTÓN: Ancho total en móvil, al final en PC */}
            <button 
      ...
      <main className="max-w-7xl mx-auto px-6 pt-12">
        <div className="mb-12">
          ...
          <p className="text-slate-500 font-medium">Gestiona tu inventario con un diseño de alta gama.</p>
        </div>
              ...
              <div key={p.id} className="group animate-fade-in">
                <div className="relative aspect-[4/5] mb-5 overflow-hidden rounded-[3rem] bg-slate-200 shadow-sm border border-slate-100 transition-all group-hover:shadow-2xl">
                  ...
                  <div className="absolute bottom-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                    <button onClick={() => handleEditClick(p)} className="bg-white/90 backdrop-blur-md p-3 rounded-full text-blue-600 hover:bg-blue-600 hover:text-white shadow-xl transition-all">
                      <Edit2 size={18} />
                    </button>
                    <button onClick={() => deleteProduct(p.id)} className="bg-white/90 backdrop-blur-md p-3 rounded-full text-red-500 hover:bg-red-500 hover:text-white shadow-xl transition-all">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </div>
      ...
      {/* MODAL INTELIGENTE (ADD / EDIT) */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-lg rounded-[3rem] p-12 shadow-2xl relative max-h-[90vh] overflow-y-auto no-scrollbar">
            <button onClick={() => setIsModalOpen(false)} className="absolute top-10 right-10 text-slate-300 hover:text-slate-900"><X size={28} /></button>
